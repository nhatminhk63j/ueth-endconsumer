...
export const TableRow = withStyles(
  () => ({
    root: {
      "&:hover > td": { background: NEUTRAL_100 },
    },
  }),
  { index: 1 }
)(TableRowRaw);
...
export interface Column<T = any> {
  ...
  render?: (col: T, index: number) => JSX.Element;
  fixed?: "right" | "left";
  width?: number;
  hidden?: boolean;
  ...
  renderHeader?: (col: some, index: number) => JSX.Element;
}
...
const TableCustom = <T,>(props: Props<T>) => {
  ...
  return (
    ...
    >
      {header}
      <div
        className={`${classes.cover} ${className?.cover}`}
        style={{ borderRadius: header ? "0px" : "8px" }}
      >
        <TableContainer
          ref={container}
        ...
        >
          ...
          >
            ...
            <TableHead
              ...
              style={{
                display: noHeaderColumns ? "none" : undefined,
                padding: 0,
              }}
            >
